...
The mutagen data contains 1580 columns. This is a lot of features. Because the example here is for educational purposes, I decided to reduce the dataset to 100 randomly selected columns. In real life, you would perform a more regirous and substantiated feature seelction procedure. For example, you could use Ridge Regression or LASSO to select features. Or use a permutation approach in combination with a tree based algorithm. A good example on how to do this is described in [this blog by Julia Silge](https://juliasilge.com/blog/lasso-the-office/) or in [this video by Adrew Couch](https://youtu.be/1AKug0tgux8).
...
remaining_columns <- sample(2:ncol(mutagen_tbl), 99)
...
### Visualize tuning
We chose a small grid of only 4 points for each tuning parameter and a low number of trees to keep the run time low. Note that if you increase these number, run time will become slower up to the point that you either need to wait for several hours or run things on a larger machine. On a 30 core machine with >400 Gb of RAM, the step above took about 1.5 minutes. If you would like to time runtime, you could use [the `{tictoc}` package](https://www.jumpingrivers.com/blog/timing-in-r/). The output can be retrieved by printing `toc.log(format = TRUE)` to the console to get the elapsed time, since tic(). 
```{r}
tune_res %>%
  collect_metrics() %>%
  filter(.metric == "roc_auc") %>%
...
Based on inital (rough) tuning we can define a more fine-grained grid
```{r}
...
This step takes a long time. I ran this on a 30 core machine with >400 Gb RAM, which took about 8 hours to complete. In stead of running we load the resulting R-object from disk. I commented the code out, so that it cannot be run accidentally.
```{r, eval=FALSE}
 registerDoMC(cores = max(1, parallelly::availableCores() - 1))  
 tictoc::tic()
 set.seed(456)
 ...
 ## get timings
 tictoc::tic.log(format = TRUE)
 write_rds(regular_res, here::here(base_folder, "data", "regular_res.rds"))
```
...
Not too bad, and better then logis
